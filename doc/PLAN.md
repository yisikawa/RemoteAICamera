...
> 作成: 2026-06-01 / 最終更新: 2026-06-10 (日次集計カメラ別ヒストグラム・積み上げ表示・削除基準修正)  
...
| 日次集計タブ「すべて」：全カテゴリ積み上げ＋カメラ別積み上げ＋6カテゴリグリッド | ✅ |
| 日次集計タブ カテゴリ選択：サブカテゴリ積み重ね＋未分類補完＋サブカテゴリ別日次グリッド | ✅ |
| 日次集計タブ カメラ別ヒストグラム（全体・カテゴリ別・サブカテゴリ対応） | ✅ |
| 日次集計タブ：直近 7/14/30 日期間切り替え | ✅ |
| イベント一覧：サブカテゴリドロップダウンフィルター（カメラ別） | ✅ |
| イベント一覧：↑↓キーによるレコード移動（カメラ内） | ✅ |
| 詳細パネル：種別・サブカテゴリのクリック編集 | ✅ |
| 古いイベント一括削除：72時間→日付単位（3日前の日付以前）に変更 | ✅ |
| 人物管理・車両管理・チャットUI | 🔲 将来 |
...
| `GET /api/stats/daily-camera?days=14` | 日別×カメラ×カテゴリ件数集計 |
| `GET /api/stats/daily-sub-camera?detection_type=&days=14` | 日別×カメラ×サブカテゴリ件数集計（detection_type 指定必須） |
