# RemoteAICamera

TP-Link Tapo C520W カメラ + ローカル AI (LM Studio) を使った、プライベート監視・人物/車両認識・記録システム。

クラウド不使用・完全ローカル動作を優先設計。

---

## 機能概要

| フェーズ | 機能 | 状態 |
|---|---|---|
| Phase 1 | RTSP受信・ONVIF駆動イベント検知・YOLOv8分類・静止画/動画保存・SQLite記録 | ✅ 完了 |
| Phase 2 | 顔認識 (InsightFace)・ナンバープレート認識 (EasyOCR) | 🔲 予定 |
| Phase 3 | Ollama連携・カテゴリ別画像類似判定（Qwen2.5-VL）・サブカテゴリ分類 | ✅ 完了 |
| Phase 4 | Web ダッシュボード (FastAPI + React)・日次集計・UI改善 | ✅ 完了 |

---

## システム構成

```
Tapo C520W
 ├─[RTSP]────► RTSPCapture (常時受信・20秒リングバッファ)
 │
 └─[ONVIF]───► poll_events() (2秒ポーリング)
                     │
             [検知シグナル]
                     │
             handle_clip() ← 別スレッド
                     │
        ┌────────────┴──────────────────┐
        │ sleep(12s) ← ポストロール     │
        │ get_recent_frames(15s)        │
        │ ClipAnalyzer.analyze()        │
        │ → YOLO 全80クラス推論         │
        │ → 6カテゴリ分類               │
        │   (人/車/バイク/自転車/ペット/その他)
        │ → best_frame + detections保存 │
        │ EventStore.save_event()       │
        └───────────────────────────────┘
                     │
             FastAPI サーバー (port 8000)
                     │
             React ダッシュボード
```

複数カメラは各自スレッドで並列処理。YOLOv8モデルは全カメラで共有 (VRAM節約)。

---

## YOLO 検出カテゴリ

YOLOv8s (COCO 80クラス) の検出結果を以下の6カテゴリに集約します。

| カテゴリ | COCOクラス |
|---|---|
| 人 (person) | person (0) |
| 車 (car) | car (2), bus (5), truck (7) |
| バイク (motorcycle) | motorcycle (3) |
| 自転車 (bicycle) | bicycle (1) |
| ペット (pet) | bird (14), cat (15), dog (16) |
| その他 (other) | 上記以外の検出 |

`config.yaml` の `target_classes: [0, 1, 2, 3, 5, 7, 14, 15, 16]` により対象外クラスの推論を省略。  
夜間IR映像では traffic cone 等が person に誤分類される場合あり（`tools/cleanup_*.py` で削除可能）。

---

## 動作環境

| 項目 | 要件 |
|---|---|
| OS | Windows 11 |
| Python | 3.11 以上 |
| GPU | NVIDIA RTX シリーズ (CUDA 12.x) |
| カメラ | TP-Link Tapo C520W (RTSP/ONVIF対応) |
| AI推論 | Ollama (Qwen2.5-VL 7B, Phase 3) |

開発・検証環境: RTX 4060 16GB / CUDA 12.8 / PyTorch cu126

---

## セットアップ

### 1. リポジトリのクローン

```bash
git clone https://github.com/yisikawa/RemoteAICamera.git
cd RemoteAICamera
```

### 2. 仮想環境の作成

```bash
python -m venv .venv
.venv\Scripts\activate
```

### 3. 依存パッケージのインストール

`setup.bat` を実行します (PyTorch CUDA + 全パッケージを一括インストール)。

```bat
setup.bat
```

手動で行う場合:

```bash
# PyTorch (CUDA 12.6 ホイール / CUDA 12.8 互換)
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126

# その他パッケージ
pip install -r requirements.txt
```

### 4. フロントエンドのビルド

```bash
cd frontend
npm install
npm run build
cd ..
```

### 5. 設定ファイルの作成

```bash
copy config.yaml.example config.yaml
```

`config.yaml` を編集してカメラ情報を入力します。

```yaml
cameras:
  - name: "frontCamera"
    host: "192.168.1.100"           # カメラの IP アドレス
    stream_username: "admin"        # RTSPユーザー名
    stream_password: "your_password"
    onvif_port: 2020                # Tapo C520W: 2020
    rtsp_stream: 1
```

> **カメラIPの確認方法**
> ```bash
> python discover.py --password YOUR_PASSWORD
> ```

### 6. 起動

```bash
python main.py
```

ブラウザで `http://localhost:8000` を開くとダッシュボードが表示されます。

---

## ディレクトリ構成

```
RemoteAICamera/
├── camera/
│   ├── tapo_client.py      # ONVIF接続・イベント取得
│   └── rtsp_capture.py     # RTSP受信・自動再接続・フレームバッファ
├── pipeline/
│   ├── detector.py         # YOLOv8 GPU推論・80クラス・6カテゴリマッピング
│   ├── clip_analyzer.py    # 15秒クリップ分析・ベストフレーム抽出
│   └── event_filter.py     # イベントフィルタリング
├── ai/
│   ├── identity_client.py  # 画像類似判定クライアント (Ollama)
│   └── sub_category_client.py  # サブカテゴリ分類クライアント (Ollama)
├── api/
│   ├── server.py           # FastAPI アプリ
│   ├── ws_manager.py       # WebSocket リアルタイム通知
│   └── routes/             # REST API エンドポイント
├── frontend/
│   ├── src/                # React + TailwindCSS ソース
│   └── dist/               # ビルド成果物 (gitignore)
├── db/
│   ├── models.py           # SQLite ORM モデル定義
│   └── store.py            # DB操作ファサード
├── storage/
│   └── file_store.py       # 静止画・動画クリップ保存
├── data/                   # (gitignore) 実行時に自動生成
│   ├── snapshots/          # 静止画 (日付フォルダ別)
│   ├── clips/              # 動画クリップ (日付フォルダ別)
│   ├── models/             # YOLOv8 ウェイトキャッシュ
│   └── events.db           # SQLite データベース
├── tools/
│   ├── cleanup_other.py        # その他誤検知削除ツール
│   ├── cleanup_false_person.py # 人誤検知削除ツール
│   └── cleanup_false_car.py    # 車誤検知削除ツール
├── main.py                 # エントリーポイント
├── config.py               # 設定ファイルローダー
├── discover.py             # カメラ IP 自動検出ツール
├── config.yaml             # (gitignore) 実際の設定 (パスワード含む)
├── config.yaml.example     # 設定テンプレート
├── requirements.txt        # 依存パッケージ一覧
└── setup.bat               # Windows セットアップスクリプト
```

---

## Web ダッシュボード

`http://localhost:8000` でアクセスできます。

| 機能 | 説明 |
|---|---|
| カテゴリフィルター | 人/車/バイク/自転車/ペット/その他 をクリックで絞り込み |
| イベント一覧 | カメラ別・時系列でイベントを表示（全件） |
| サブカテゴリフィルター | カテゴリ選択時にカメラ別ドロップダウンでサブカテゴリ絞り込み |
| キーボードナビゲーション | ↑↓キーで同カメラ内のレコードを移動 |
| 詳細パネル | スナップショット・クリップ動画・検出情報・サブカテゴリを表示 |
| 種別・サブカテゴリ編集 | 詳細パネルでクリックして手動修正可能 |
| イベント削除 | 個別削除（DB＋ファイル）・3日以上前の一括削除 |
| 類似検索 | 同カテゴリのスナップショットをLLMで比較し同一対象を検出（双方向対応） |
| 日次集計タブ | 直近 7/14/30 日の期間切り替え。すべてビューでは全カテゴリ積み上げ＋カメラ別積み上げ＋6カテゴリグリッド。カテゴリ選択時はサブカテゴリ積み重ねヒストグラム＋カメラ別ヒストグラム（サブカテゴリ対応）＋サブカテゴリ別日次グリッドを表示 |
| 統計タブ | カテゴリ別サブカテゴリヒストグラム・LLMによるバッチ分類（件数降順・ゼロ補完表示） |

### イベント一覧・類似検索画面

![通常＋類似検索画面](doc/通常＋類似検索画面.png)

### 日次集計タブ

![日次集計画面](doc/日次集計画面.png)

#### 「すべて」表示（カテゴリ未選択時）

| ブロック | 内容 |
|---|---|
| 全カテゴリ積み上げ | 日付×全6カテゴリの積み上げ棒グラフ・合計件数表示 |
| カメラ別積み上げ | カメラごとに色分けした積み上げ棒グラフ（複数カメラ時のみ表示） |
| 6カテゴリグリッド | 各カテゴリの単独棒グラフをグリッド配置 |

#### カテゴリ選択時

| ブロック | 内容 |
|---|---|
| サブカテゴリ積み重ねヒストグラム | カテゴリ合計棒の中をサブカテゴリ色で積み重ね表示。未分類分は薄いカテゴリ色で補完 |
| カメラ別ヒストグラム | カメラ別にサブカテゴリ色で積み重ね表示（サブカテゴリ分類済み時） |
| サブカテゴリ別日次グリッド | 各サブカテゴリの独立棒グラフをグリッド配置。合計件数付き |
| サブカテゴリ分類ボタン | 未分類イベントを Ollama でバッチ分類・進捗バー付き |

### 統計タブ

![統計画面](doc/統計画面し.png)

---

## カメラの追加

`config.yaml` に `cameras:` ブロックを追加するだけで、自動的に並列スレッドで処理されます。

```yaml
cameras:
  - name: "eastCamera"
    host: "192.168.1.100"
    ...
  - name: "southCamera"
    host: "192.168.1.101"
    ...
```

---

## VRAM 使用量の目安 (RTX 4060 16GB)

| モデル | 使用量 |
|---|---|
| YOLOv8s (常駐) | ~0.5 GB |
| InsightFace buffalo_l (Phase 2) | ~1.0 GB |
| EasyOCR (Phase 2) | ~1.5 GB |
| Qwen2.5-VL 7B via Ollama (Phase 3) | ~6.0 GB |
| **合計 (最大)** | **~9.0 GB** |

---

## ロードマップ

### Phase 2: 顔認識・ナンバープレート認識
- InsightFace による顔エンコーディング登録・照合
- EasyOCR / PaddleOCR による日本語ナンバープレート認識
- 既知人物・既知車両の通過ログ蓄積

### ✅ Phase 3: Ollama ローカル AI 連携（完了）
- Ollama + Qwen2.5-VL 7B による同カテゴリスナップショット類似判定（SAME/DIFFERENT）
- 類似検索 SSE API・ダッシュボード「類似を検索」UI（双方向対応・上限 300 件）
- 類似判定結果を `event_similarities` テーブルに保存
- カテゴリ別サブカテゴリ分類（`sub_category_client.py`）・統計タブヒストグラム表示

### ✅ Phase 4: Web ダッシュボード UI 改善（完了）
- 日次集計タブ：直近7/14/30日切り替え・全カテゴリ積み上げ・カメラ別積み上げ・6カテゴリグリッド
- 日次集計タブ（カテゴリ選択時）：サブカテゴリ積み重ねヒストグラム・カメラ別サブカテゴリヒストグラム・サブカテゴリ別日次グリッド・未分類補完表示
- サブカテゴリフィルター：カメラ別ドロップダウン（カテゴリ選択時のみ表示）
- ↑↓キーナビゲーション：同カメラ内レコードをキーボードで移動・自動スクロール
- 詳細パネル：サブカテゴリ表示・クリックで手動編集（種別と同様）
- 古いイベント一括削除：3日前の日付以前を基準に削除
- 誤検知クリーンアップツール群（`tools/cleanup_*.py`）

### Phase 5 以降（予定）
- 自然言語クエリ対応（「昨日Aさんは何時に来た？」）
- 日次イベントレポート自動生成

---

## 注意事項

- 本システムで取得する映像・顔情報・車両情報は**個人情報保護法**の対象となる場合があります
- **私有地内・私的利用**の範囲で使用してください
- pytapo は非公式ライブラリのため、カメラのファームウェア更新で動作が変わる可能性があります

---

## ライセンス

MIT License
